...
func (r *postRepo) Create(userID int, content string) (*model.Post, error) {
	post := &model.Post{}
	err := r.db.QueryRow(
...
func (r *postRepo) GetByID(id, currentUserID int) (*model.Post, error) {
	post := &model.Post{}
	err := r.db.QueryRow(
		`SELECT p.id, p.user_id, u.username, u.avatar_url, p.content,
	...
	).Scan(&post.ID, &post.UserID, &post.Username, &post.AvatarURL, &post.Content,
...
func scanPosts(rows *sql.Rows) ([]*model.Post, error) {
	...
	for rows.Next() {
		...
		err := rows.Scan(&post.ID, &post.UserID, &post.Username, &post.AvatarURL,
			&post.Content, &post.LikesCount, &post.IsLiked, &post.CreatedAt, &post.UpdatedAt)
		if err != nil {
			return nil, err
		}
